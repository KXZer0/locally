...
locally's routing assumes text->NPU (LLMPipeline) and images->GPU
...
def is_vlm(model_dir):
    """Mirror of locally.is_vlm() — config.json architecture/model_type sniff."""
...
def main():
    ...
    print("=== locally NPU multimodal probe ===")
    ...
    print(f"is_vlm()     {vlm}  -> locally would route this as {'VLM' if vlm else 'LLM'}")
    ...
    if llm_ok:
        ...
        if not vlm:
            ...
            print("  NOTE: is_vlm() says VLM, but text via LLMPipeline works. locally would")
