...
# install.ps1 — locally setup: venv, dependencies, terminal model selection
#
# Usage:
#     .\install.ps1                       # terminal model selection
#     .\install.ps1 -SkipModel            # venv + deps only
#     .\install.ps1 -TerminalSetup        # legacy terminal model chooser
#     .\install.ps1 -HfToken hf_xxx       # auth for gated/private models
#
# Setup and inference run in the terminal. No browser is launched.
#
...
Write-Host ""
Write-Host "=== locally Install ===" -ForegroundColor Cyan
Write-Host ""
...
# Mirror locally.py's detect_devices(): canonical-keyed {kind: {id, name}}.
...
if (Test-Path $ModelsRoot) {
    ...
    $LocalModels = @(Get-ChildItem -Path $ModelsRoot -Directory | Where-Object {
        ...
        # Mirror locally.py is_vlm(): the definitive VLM signal is the
...
# locally runs ONE primary model + (optionally) ONE GPU secondary, so the combos
...
Write-Host ""
Write-Host "=== What will you use locally for? ===" -ForegroundColor Cyan
Write-Host ""
$cases = @()
...
if ($HasGPU) {
    ...
    $cases += [PSCustomObject]@{ Key = "chat+vision"; Label = "[$comboChatDev] Chat + [GPU] Vision";       Desc = "two models, one server (the classic locally setup)" }
}
...
$ArgsLiteral = (($StartArgs | ForEach-Object {
    "'" + ($_ -replace "'", "''") + "'"
}) -join " ")

# Generate a PowerShell-5.1-compatible launcher. The installer needs
# PowerShell 7; the generated one has to run on the Windows default shell.
#
# It calls the venv's Python directly rather than forwarding through
# locally-launch.ps1. That script's -ExtraArgs is declared
# ValueFromRemainingArguments, and binding it EXPLICITLY closes the door on
# everything else: `./start.ps1 --device GPU` died with "A parameter cannot be
# found that matches parameter name '-device'" on both 5.1 and 7, and only
# --port survived because it happened to bind to -Port. Verified on both
# hosts. With nothing to bind there is nothing to break, and argparse takes
# the last flag, so a user override still wins over the installed default.
$Content = "# Auto-generated by install.ps1`n"
$Content += "`$ErrorActionPreference = 'Stop'`n"
$Content += "`$Python = if (`$env:OS -eq 'Windows_NT') { Join-Path `$PSScriptRoot 'venv\Scripts\python.exe' } else { Join-Path `$PSScriptRoot 'venv/bin/python' }`n"
$Content += "if (-not (Test-Path -LiteralPath `$Python)) { throw 'Run install.ps1 first to create the server environment.' }`n"
$Content += "& `$Python (Join-Path `$PSScriptRoot 'locally.py') $ArgsLiteral @args`n"
$Content += "exit `$LASTEXITCODE`n"
