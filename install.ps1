...
# install.ps1 — locally setup: venv, dependencies, guided browser onboarding
#
# Usage:
#     .\install.ps1                       # guided setup in the locally UI
...
#
# The default stops doing setup in the terminal as soon as dependencies are
# ready. locally starts without a model, measures the machine, and walks the
# user through assistant, voice and web choices in the browser. The original
# terminal chooser remains available for unattended/legacy workflows.
#
...
# ---------------------------------------------------------------------------
# Default path: hand first-run setup to the UI
# ---------------------------------------------------------------------------

if (-not $SkipModel -and -not $TerminalSetup) {
    # start.ps1 is machine-local and ignored by Git. Keep it portable within
    # the clone: scripts/locally-launch.ps1 reads the setup choice persisted by
    # the UI and also opens onboarding again when no choice exists yet.
    $StartScript = Join-Path $ScriptDir "start.ps1"
    $Content = "# Auto-generated by install.ps1`n"
    $Content += "& (Join-Path `$PSScriptRoot 'scripts\locally-launch.ps1') @args"
    Set-Content -Path $StartScript -Value $Content -Encoding UTF8
    Write-Host "[OK] Generated start.ps1" -ForegroundColor Green
    Write-Host "Opening locally's guided setup..." -ForegroundColor Cyan
    Write-Host "You can return to it later from Settings > Device setup." -ForegroundColor DarkGray
    Pop-Location
    & (Join-Path $ScriptDir "scripts\locally-launch.ps1")
    exit $LASTEXITCODE
}
