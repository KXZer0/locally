#!/usr/bin/env python3
"""locally benchmark — compare workloads across devices and backends.

Usage:
    # locally server (auto-detects devices/models from /health):
    ...
    #   2. NPU:       (start locally --device NPU)  benchmark.py --label npu
    #   3. iGPU:      (start locally --device GPU)  benchmark.py --label igpu
    #   4. CPU:       (start locally --device CPU)  benchmark.py --label cpu
...
# Pass them via --same-1/--same-2/--diff-1/--diff-2 or set LOCALLY_IMAGES_DIR
# and put matching files named same-1.jpg, same-2.jpg, diff-1.jpg, diff-2.jpg.
DEFAULT_IMAGES_DIR = os.environ.get("LOCALLY_IMAGES_DIR", "")
...
def stream_request(url, body):
    ...
    channel) as tokens. locally doesn't separate <think> from content, so
...
def main():
    ...
    parser.add_argument("--backend", choices=["locally", "ollama"], default="locally",
                        help="Server type. locally (default) auto-detects via /health; "
                             "ollama uses --model directly and skips health check.")
    parser.add_argument("--url", default=None,
                        help="Server URL. Default: http://localhost:8000 for locally, "
                             "http://localhost:11434 for ollama.")
    parser.add_argument("--model", default=None,
                        help="Model name to test (required for --backend ollama, "
                             "optional override for locally).")
    ...
    if args.backend == "ollama":
        ...
        if args.vlm_only:
            ...
            sys.exit(1)
        # Ollama is text-only here — VLM tests are locally-specific
        args.llm_only = True
    ...
    if args.backend == "ollama":
        ...
        # locally: use /health to discover devices and types
        try:
