...
`docs/archive/REBUILD-PLAN.md` §2.3 says "One `AbortController` per view; register
...
`docs/archive/ui-tooling/scripts/uidrive.mjs`):
...
is the test that will show it: re-run `docs/archive/ui-tooling/scripts/probes/listeners.js`.
...
`docs/archive/ui-tooling/scripts/css-oracle.js` already warns that `innerWidth` is **0** in a hidden
...
**Verdict: the rig owns the browser.** `docs/archive/ui-tooling/scripts/uidrive.mjs` launches the
